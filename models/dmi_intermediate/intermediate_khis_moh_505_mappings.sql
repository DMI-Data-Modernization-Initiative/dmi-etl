select
    ...
    end as indicator_value,  /* handle non-text values so that we have just intergers */
...
where
    ...
    and case when indicator_value ~ '^\d+$' then indicator_value::int else null end
    is not null  /* handle non-text values so that we have just intergers */
