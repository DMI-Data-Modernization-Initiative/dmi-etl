with subset_data as (
	select 
		...
		case 
			when sex::int = 1 then 'Female'
			when sex::int = 0 then 'Male'
		end as sex,
		barcode::int
...
left join {{ ref( 'dim_age_group') }} as age_group on subset_data.calculated_age >= age_group.start_age 
