...
select
    ...
    sum(enrolled)
    - sum(case when flutest = 1 or covidtest::int = 1 then 1 else 0 end) as not_tested,
    sum(
        case when flutest = 1 and covidtest = 1 then 1 else 0 end
    ) as tested_for_influenza_sars_cov_2,
    sum(
        case when flutest = 1 and covidtest = 0 then 1 else 0 end
    ) as tested_for_influenza_only,
    sum(
        case when flutest = 0 and covidtest = 1 then 1 else 0 end
    ) as tested_for_sars_cov_2_only,
    ...
    sum(
        case when flub_positive = 0 and flua_positive = 1 then 1 else 0 end
    ) as influenza_a,
    sum(
        case when flub_positive = 1 and flua_positive = 0 then 1 else 0 end
    ) as influenza_b,
    sum(
        case when flub_positive = 1 and flua_positive = 1 then 1 else 0 end
    ) as influenza_a_b,
    sum(
        case
            when flua_positive = 1 and flub_positive = 0 and ph1n1 = 1 then 1 else 0
        end
    ) as a_h1n1,
    sum(
        case when flua_positive = 1 and flub_positive = 0 and h3n2 = 1 then 1 else 0 end
    ) as a_h3n2,
    sum(
        case
            when flua_positive = 1 and flub_positive = 0 and unsub_non = 1 then 1 else 0
        end
    ) as non_sub_typable,
    sum(
        case
            when flua_positive = 1 and flub_positive = 0 and notsubtyp = 1 then 1 else 0
        end
    ) as not_yet_sub_typed,
    sum(
        case
            when flua_positive = 0 and flub_positive = 1 and victoria = 1 then 1 else 0
        end
    ) as b_victoria,
    sum(
        case
            when flua_positive = 0 and flub_positive = 1 and yamagata = 1 then 1 else 0
        end
    ) as b_yamagata,
    sum(
        case
            when flua_positive = 0 and flub_positive = 1 and flub_undetermined = 1
            then 1
            else 0
        end
    ) as not_determined,
...
left join
    ...
    on facility.facility_name_short = sari_ili.facility_name
left join
    {{ ref("dim_date") }} as screen_date on screen_date."date" = sari_ili.date_screened
left join
    {{ ref("dim_patient_outcome") }} as pat_outcome
    on pat_outcome.outcome = sari_ili.outcome
left join
    {{ ref("dim_age_group_sari") }} as age_group
    on sari_ili.age_in_years >= age_group.start_age
